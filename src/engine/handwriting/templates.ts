 ...
 */
const ALTERNATES: Record<string, () => TplPoint[][]> = {
  // G d'un seul trait : la boucle se referme puis remonte vers la barre
  G: () =>
    pen()
      .a(0.45, 0.5, 0.45, 0.5, 60, 305)
      .m(0.68, 0.93)
      .l(0.92, 0.56)
      .l(0.5, 0.56)
      .done(),
  ...
  R: () =>
    pen()
      ...
      .m(0, 0)
      .l(0.4, 0)
      .c(0.76, 0, 0.76, 0.5, 0.4, 0.5)
      .l(0, 0.5)
      .m(0.3, 0.5)
      .l(0.84, 1)
      .done(),
  // U anguleux (fond presque plat), très courant à la main
  U: () => pen().m(0, 0).l(0.04, 0.72).l(0.24, 0.98).l(0.56, 1).l(0.76, 0.72).l(0.78, 0).done(),
  // D à panse très ronde
  D: () => pen().m(0, 0).l(0, 1).m(0, 0.02).c(0.72, 0.02, 0.72, 0.98, 0, 0.98).done(),
  // S plus anguleux
  S: () =>
    pen()
      .m(0.76, 0.12)
  ...
  O: () => pen().a(0.45, 0.5, 0.45, 0.5, -80, 250).done(),
}
...
/**
 * Répartit `n` points sur un ensemble de sous-tracés, proportionnellement à
 * leur longueur (jamais moins de 2 points par sous-tracé, sinon la barre du
 * « t » ou le point du « i » disparaîtraient du nuage).
 */
...
export function templates(): Template[] {
  ...
  for (const char of Object.keys(ALTERNATES)) cache.push({ char, paths: ALTERNATES[char]() })
  return cache
}
